...
<body>
  <div class="wrapper">
    ...
    <h2>Mark Entry</h2>
    <form action="select_mark_approval.php" method="post" style="min-height: 250px;">
      <div class="input-box">
        <select required name="subject">
          <option disabled selected>Select Subject</option>
          <?php
          $get_hos = "select a.subject,a.sub_code,c.CLASS_NAME from waec_subject a join teacher_subject b on (a.sub_code=b.sub_code) join sub_class c on (b.s_code=c.sub_code) order by a.subject,c.class_name";
        ...
        if (isset($_POST['change'])) {
          if (isset($_POST['subject'])) {
            if (isset($_POST['term'])) {
              ...
              $sql = oci_parse($conn, "select a.SUB_CODE,c.S_CODE from waec_subject a join teacher_subject c on (a.sub_code=c.sub_code) join sub_class d on (c.s_code=d.sub_code) where a.subject = '$subject' and d.class_name = '$className' ");
              oci_execute($sql);
        ...
        ?><div style="font-size:15px;
              ...
              <?php echo '<script>
                                    Swal.fire({
                                        position: "center",
                                        icon: "success",
                                        title: "MARKS TO BE APPROVED FOR ' . $class_name . ' TAKING ' . $subject . '",
