...
<body>
  ...
  text-decoration: none;" name="filter" type="submit">
        ...
        <div style="display:flex; margin-top:20px;">
            <Label style="font-size: 18px; font-family: righteous;
         font-weight: bold; color: #909290;">Student Billing</Label>
        </div>
  ...
  text-decoration: none;" name="filters" type="submit">
        ...
        <div class="input-container" style="display: flex;">
            <div class="input-field" style="margin-right: 10px;"></div>

            <div class="input-field" style="margin-right: 10px;">
                <label for="subjectCode">Invoice Number</label>
                <input type="text" placeholder="Enter Invoice Number" title="Only Letters and Numbers" name="inv_no" pattern="[A-z0-9 ]+">
            </div>
            ...
            <div class="input-field" style="margin-right: 10px;">
                ...
                <select required name="term">
                    ...
                    <?php
                    $get_hos = "SELECT * FROM TERM_CALENDAR WHERE STATUS = 'ACCEPTED' ";
                    $get = oci_parse(oci_connect($username, $password, $connection), $get_hos);
                    oci_execute($get);
                    while ($row = oci_fetch_array($get)) {
                    ?><option value="<?php echo $row['TERM'] ?>">
                            <?php echo trim($row["TERM"]); ?>
                        </option> <?php
                                }
                                    ?>
                </select>
            ...
            <div class="input-field" style="margin-right: 10px;">
                ...
                <select required name="pyt">
                    <option disabled selected>Select Student </option>
                    <?php
                    $sub_code = $_SESSION['sub_code'];
                    $get_hos = "SELECT * FROM STUDENT A JOIN CLASS_STUDENT B ON (A.STUD_ID=B.STUD_ID) WHERE B.SUB_CODE = $sub_code   ORDER BY A.NAME";
                    $get = oci_parse(oci_connect($username, $password, $connection), $get_hos);
                    oci_execute($get);
                    while ($row = oci_fetch_array($get)) {
                    ?><option value="<?php echo $row['STUD_ID'] ?>">
  ...
  margin-top:10px;
  margin-bottom:10px;
  text-decoration: none;" name="bill" type="submit">
                BILL STUDENT
        ...
        <div style="display:flex; margin-top:20px;">
            <Label style="font-size: 18px; font-family: righteous;
         font-weight: bold; color: #909290;">Student Miscellaneous Fees</Label>
        </div>
  ...
  border: none;
  border-radius: 4px;
  text-decoration: none;" name="m_filters" type="submit">
                    FILTER CLASS
                    <i class="uil uil-filter"></i>
                </button>
            </div>
            ...
            <?php
            if (isset($_POST['m_filters'])) {
                if (isset($_POST['m_stid'])) {
                    echo  '<script>
                            Swal.fire({
                                position: "center",
                                icon: "info",
        ...
        <div class="input-container" style="display: flex;">
            ...
            <div class="input-field" style="margin-right: 10px;">
                <label for="subjectCode">Invoice Number</label>
                <input type="text" placeholder="Enter Invoice Number" title="Only Letters and Numbers" name="m_inv_no" pattern="[A-z0-9 ]+">
            </div>

            <div class="input-field" style="margin-right: 10px;">
                <label for="subjectCode">Term</label>
                <select required name="m_term">
                    <option disabled selected>Select Term</option>
                    <?php
                    $get_hos = "SELECT * FROM TERM_CALENDAR WHERE STATUS = 'ACCEPTED' ";
                    $get = oci_parse(oci_connect($username, $password, $connection), $get_hos);
            ...
            <div class="input-field" style="margin-right: 10px;">
                ...
                <input type="text" placeholder="Enter Description" title="Only Letters and Numbers" name="m_des" pattern="[A-z0-9 ]+">
            </div>

            <div class="input-field" style="margin-right: 10px;">
                <label for="subjectCode">Student</label>
                <select required name="m_pyt">
                    <option disabled selected>Select Student </option>
                    <?php
                    $sub_code = $_SESSION['sub_code'];
                    $get_hos = "SELECT * FROM STUDENT A JOIN CLASS_STUDENT B ON (A.STUD_ID=B.STUD_ID) WHERE B.SUB_CODE = $sub_code   ORDER BY A.NAME";
            ...
            <div class="input-field" style="margin-right: 10px;">
                <label for="subjectCode">Amount</label>
                <input type="number" placeholder="Enter Amount" title="Only Numbers" name="m_amt">
            </div>
  ...
  margin-top:10px;
  margin-bottom:10px;
  text-decoration: none;" name="m_bill" type="submit">
                BILL STUDENT
                <i class="uil uil-bill"></i>
            </button>
        </div>
        <?php
        if (isset($_POST['m_bill'])) {
            $sub_code = $_SESSION['sub_code'];
            $inv = $_POST['m_inv_no'];
            $des = $_POST['m_des'];
            if ($inv != '') {
                if ($des != '') {
                    if (isset($_POST['m_term'])) {
                        $t = $_POST['m_term'];
