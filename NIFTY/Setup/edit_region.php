...
<body>
    <div class="wrapper">
        ...
        <h2>Edit Region</h2>
        <form action="edit_region.php" id="form" method="post">
            ...
            <div class="input-box username">
                <input type="text" placeholder="Update Region" required pattern="[A-z0-9 ]+" title="Only Letters And Numbers" name="upd_reg">
            </div>
            ...
            animation:button .3s linear;text-align: center;">
                            <?php ?>
                                ...
                                $check = oci_parse($conn, "select * from region where region='$up_reg'");
                                oci_execute($check);
                ...
                position: relative;
                 display:flex;
                animation:button .3s linear;text-align: center;">
                                <?php echo "REGION ALREADY EXIST";
                                    header("refresh:3;"); ?>
                            </div> <?php
                                } else {
                                  
                                    $sql = oci_parse($conn, "update region set region = '$up_reg' where reg_code = '$reg_code'");
                                    oci_execute($sql);
                                    $check = oci_parse($conn, "select * from region where region='$up_reg'");
                                    oci_execute($check);
                                    if (oci_fetch_all($check, $a) > 0) {
