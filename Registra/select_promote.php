  ...
  <div class="wrapper">
    ...
    <form action="select_promote.php" method="post" style="min-height: 250px;">
      ...
      <select required name="teacher">
                ...
                <?php
                $get_hos = "select distinct(a.class_name) from sub_class a join class_student  b on (a.sub_code=b.sub_code) where a.s_id = $sid order by a.class_name";
      ...
      <div class="input-box">
        <select required name="acad_year">
          ...
          <?php
          $get_hos = "SELECT DISTINCT(B.ACADEMIC_YEAR) FROM STUDENT_STANDINGS A JOIN TERM_CALENDAR B ON (A.TERM=B.TERM) ORDER BY B.ACADEMIC_YEAR";
          $get = oci_parse(oci_connect($username, $password, $connection), $get_hos);
          oci_execute($get);
          while ($row = oci_fetch_array($get)) {
          ?><option>
        ...
        if (isset($_POST['change'])) {
                if(isset($_POST['teacher'])) {
                    ...
                    while($r= oci_fetch_array($sql)) {
                     ...
                     $_SESSION['academic_year'] = $_POST['acad_year'];
                   //  $_SESSION['year'] = $a_y;
