                    ...
                    if (isset($_POST['reg'])) {
                        ...
                        foreach ($stud_ids as $st) {
                            ...
                            if ($row = oci_fetch_assoc($stmts)) {
                                ...
                                $s = "SELECT DISTINCT(A.SUBJECT),ROUND(c.CONST_ASS,0) AS CA,ROUND(c.EXAM,0) AS EXAM , ROUND(D.MARK,0) AS TOTAL,F.GRADE,D.CREDIT_HRS,D.GPA FROM WAEC_SUBJECT A JOIN STUDENT_SUBJECT B ON (A.SUB_CODE=B.SUB_CODE) JOIN STUDENT_EVALUATION C ON (B.SUB_CODE=C.SUB_CODE) JOIN STUDENT_CUMULATIVE D ON (A.SUB_CODE=D.SUBJ_CODE) JOIN STUDENT E ON (D.STUD_ID=E.STUD_ID) JOIN GRADE F ON (D.G_CODE=F.G_CODE)  WHERE C.STUD_ID = '$st' AND D.STUD_ID = '$st' and b.stud_id= '$st'  AND D.ACADEMIC_YEAR ='$a_y' and C.term = '$t'  AND C.ACADEMIC_YEAR ='$a_y' and D.term = '$t' AND C.MARK_STATUS ='ACCEPTED' AND C.CLASS_CODE=$sub_code AND D.SUB_CODE = $sub_code ORDER BY A.SUBJECT ";
                                ...
                                $sql = "SELECT B.GPA AS TOTALSTU FROM STUDENT A JOIN STUDENT_STANDINGS B ON (A.STUD_ID=B.STUD_ID) WHERE A.STUD_ID = '$st' AND B.ACADEMIC_YEAR ='$a_y' and B.term = '$t' AND B.class_code = $sub_code GROUP BY B.GPA, a.STUD_ID,b.class_code,b.academic_year,b.term
                                ORDER BY B.GPA ";
                                //   echo $sql;
                                $stmt = oci_parse($conn, $sql);
                                oci_execute($stmt);
                                ...
                                //      echo $sql;
                                $sql = "SELECT B.GPA,COUNT(B.GPA) AS COUNT FROM STUDENT A JOIN STUDENT_STANDINGS B ON (A.STUD_ID=B.STUD_ID) WHERE A.STUD_ID = '$st' GROUP BY B.GPA ORDER BY B.GPA";
                                //      echo $sql;
                                $stmt = oci_parse($conn, $sql);
                                oci_execute($stmt);
                                $pdf->Ln();
